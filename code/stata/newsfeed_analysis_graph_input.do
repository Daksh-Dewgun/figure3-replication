...
* Create a temporary file to store p-values and coefficients
tempname p_values_file
tempfile p_values_filepath
file open `p_values_file' using `p_values_filepath', write replace
file write `p_values_file' "regression_name,p_value,coefficient" _n
...
reghdfe cons_news TabSource_num $controls if news_acc == 1, absorb(id wave) cluster(id) 
local b_tab = _b[TabSource_num]
local se_tab = _se[TabSource_num]
local df = e(df_r)
local t = `b_tab'/`se_tab'
local pval = 2*ttail(`df', abs(`t'))
file write `p_values_file' "cons_news_over_p,`pval',`b_tab'" _n
...
export delimited using "../../regression_outputs/newsfeed_regression_p_values.csv", replace
restore
